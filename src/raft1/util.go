...
func NewApplyHelper(applyCh chan raftapi.ApplyMsg, lastApplied int) *ApplyHelper {
	applyHelper := &ApplyHelper{
		applyCh:       applyCh,
		lastItemIndex: lastApplied,
		q:             make([]raftapi.ApplyMsg, 0),
	}
...
func (applyHelper *ApplyHelper) tryApply(msg *raftapi.ApplyMsg) bool {
	applyHelper.mu.Lock()
